...
class Environment(tk.Tk, object):
    def __init__(self):
        ...
        self.title('Bus Route. Fayza Apriliza')
    ...
    def build_environment(self):
        ...
        # Obstacle type 1 - moqsuea
        img_obstacle1 = Image.open("images/mosque.png")
        self.obstacle1_object = ImageTk.PhotoImage(img_obstacle1)
        # Obstacle type 2 - tree3
        img_obstacle2 = Image.open("images/tree3.png")
        self.obstacle2_object = ImageTk.PhotoImage(img_obstacle2)
        # Obstacle type 3 - tree4
        img_obstacle3 = Image.open("images/tree4.png")
        self.obstacle3_object = ImageTk.PhotoImage(img_obstacle3)
        # Obstacle type 4 - home
        img_obstacle4 = Image.open("images/home.png")
        self.obstacle4_object = ImageTk.PhotoImage(img_obstacle4)
        # Obstacle type 5 - constitution
        img_obstacle5 = Image.open("images/constitution.png")
        self.obstacle5_object = ImageTk.PhotoImage(img_obstacle5)
        # Obstacle type 6 - sekolah
        img_obstacle6 = Image.open("images/sekolah.png")
        ...
        # Obstacle type 8 - shop2
        img_obstacle8 = Image.open("images/shop2.png")
        self.obstacle8_object = ImageTk.PhotoImage(img_obstacle8)
        # Obstacle type 9 - apartment
        img_obstacle9 = Image.open("images/apartment.png")
        self.obstacle9_object = ImageTk.PhotoImage(img_obstacle9)
        # Obstacle type 10 - church
        img_obstacle10 = Image.open("images/church.png")
        self.obstacle10_object = ImageTk.PhotoImage(img_obstacle10)
        # Obstacle type 11 - pedagang1
        img_obstacle11 = Image.open("images/pedagang1.png")
        self.obstacle11_object = ImageTk.PhotoImage(img_obstacle11)
        # Obstacle type 12 - pedagang2
        img_obstacle12 = Image.open("images/pedagang2.png")
        self.obstacle12_object = ImageTk.PhotoImage(img_obstacle12)
        # Obstacle type 13 - pedagang3
        img_obstacle13 = Image.open("images/pedagang3.png")
        self.obstacle13_object = ImageTk.PhotoImage(img_obstacle13)

        # Creating obstacles themselves
        # Obstacles from 1 to 63
        ...
        # Obstacle 5
        self.obstacle5 = self.canvas_widget.create_image(pixels * 4, 0, anchor='nw', image=self.obstacle8_object)
        # Obstacle 6
        self.obstacle6 = self.canvas_widget.create_image(pixels * 5, pixels * 3, anchor='nw', image=self.obstacle7_object)
        # Obstacle 7
        self.obstacle7 = self.canvas_widget.create_image(pixels * 7, pixels * 3, anchor='nw', image=self.obstacle7_object)
        # Obstacle 8
        self.obstacle8 = self.canvas_widget.create_image(pixels * 6, pixels, anchor='nw', image=self.obstacle3_object)
        # Obstacle 9
        ...
        # Obstacle 18
        self.obstacle18 = self.canvas_widget.create_image(pixels, pixels * 6, anchor='nw', image=self.obstacle5_object)
        # Obstacle 19
        ...
        # Obstacle 23
        self.obstacle23 = self.canvas_widget.create_image(pixels * 3, pixels * 12, anchor='nw', image=self.obstacle5_object)
        # Obstacle 24
        self.obstacle24 = self.canvas_widget.create_image(0, pixels * 14, anchor='nw', image=self.obstacle2_object)
        # Obstacle 25
        self.obstacle25 = self.canvas_widget.create_image(0, pixels * 13, anchor='nw', image=self.obstacle2_object)
        # Obstacle 26
        self.obstacle26 = self.canvas_widget.create_image(0, pixels * 12, anchor='nw', image=self.obstacle2_object)
        # Obstacle 27
        self.obstacle27 = self.canvas_widget.create_image(0, pixels * 11, anchor='nw', image=self.obstacle2_object)
        # Obstacle 28
        self.obstacle28 = self.canvas_widget.create_image(pixels, pixels * 14, anchor='nw', image=self.obstacle2_object)
        # Obstacle 29
        self.obstacle29 = self.canvas_widget.create_image(pixels * 2, pixels * 14, anchor='nw', image=self.obstacle2_object)
        # Obstacle 30
        self.obstacle30 = self.canvas_widget.create_image(pixels * 3, pixels * 14, anchor='nw', image=self.obstacle2_object)
        # Obstacle 31
        self.obstacle31 = self.canvas_widget.create_image(pixels * 4, pixels * 12, anchor='nw', image=self.obstacle5_object)
        # Obstacle 32
        self.obstacle32 = self.canvas_widget.create_image(pixels * 5, pixels * 12, anchor='nw', image=self.obstacle5_object)
        # Obstacle 33
        self.obstacle33 = self.canvas_widget.create_image(pixels * 1, pixels * 12, anchor='nw', image=self.obstacle9_object)
        # Obstacle 34
        self.obstacle34 = self.canvas_widget.create_image(pixels * 1, pixels * 10, anchor='nw', image=self.obstacle7_object)
        # Obstacle 35
        self.obstacle35 = self.canvas_widget.create_image(pixels * 3, pixels * 8, anchor='nw', image=self.obstacle7_object)
        # Obstacle 36
        self.obstacle36 = self.canvas_widget.create_image(pixels * 1, pixels * 13, anchor='nw', image=self.obstacle8_object)
        # Obstacle 37
        self.obstacle37 = self.canvas_widget.create_image(pixels * 10, pixels * 8, anchor='nw', image=self.obstacle1_object)
        # Obstacle 38
        self.obstacle38 = self.canvas_widget.create_image(pixels * 14, pixels, anchor='nw', image=self.obstacle1_object)
        # Obstacle 39
        self.obstacle39 = self.canvas_widget.create_image(pixels * 9, pixels * 14, anchor='nw', image=self.obstacle6_object)
        # Obstacle 40
        self.obstacle40 = self.canvas_widget.create_image(pixels * 10, pixels * 4, anchor='nw', image=self.obstacle3_object)
        # Obstacle 41
        self.obstacle41 = self.canvas_widget.create_image(pixels * 10, pixels * 6, anchor='nw', image=self.obstacle3_object)
        # Obstacle 42
        self.obstacle42 = self.canvas_widget.create_image(pixels * 11, pixels * 5, anchor='nw', image=self.obstacle3_object)
        # Obstacle 43
        self.obstacle43 = self.canvas_widget.create_image(pixels * 10, pixels * 9, anchor='nw', image=self.obstacle8_object)
        # Obstacle 44
        self.obstacle44 = self.canvas_widget.create_image(pixels * 12, pixels * 11, anchor='nw', image=self.obstacle4_object)
        # Obstacle 45
        self.obstacle45 = self.canvas_widget.create_image(pixels * 11, pixels * 11, anchor='nw', image=self.obstacle4_object)
        # Obstacle 46
        self.obstacle46 = self.canvas_widget.create_image(pixels * 12, pixels * 12, anchor='nw', image=self.obstacle4_object)
        # Obstacle 47
        self.obstacle47 = self.canvas_widget.create_image(pixels * 11, pixels * 12, anchor='nw', image=self.obstacle4_object)
        # Obstacle 48
        self.obstacle48 = self.canvas_widget.create_image(pixels * 12, pixels * 13, anchor='nw', image=self.obstacle4_object)
        # Obstacle 49
        self.obstacle49 = self.canvas_widget.create_image(pixels * 11, pixels * 13, anchor='nw', image=self.obstacle4_object)
        # Obstacle 50
        self.obstacle50 = self.canvas_widget.create_image(pixels * 13, pixels * 6, anchor='nw', image=self.obstacle10_object)
        # Obstacle 51
        self.obstacle51 = self.canvas_widget.create_image(pixels * 13, pixels * 7, anchor='nw', image=self.obstacle9_object)
        # Obstacle 52
        self.obstacle52 = self.canvas_widget.create_image(pixels * 14, pixels * 5, anchor='nw', image=self.obstacle2_object)
        # Obstacle 53
        self.obstacle53 = self.canvas_widget.create_image(pixels * 14, pixels * 8, anchor='nw', image=self.obstacle2_object)
        # Obstacle 54
        self.obstacle54 = self.canvas_widget.create_image(pixels * 13, pixels * 1, anchor='nw', image=self.obstacle8_object)
        # Obstacle 55
        self.obstacle55 = self.canvas_widget.create_image(pixels * 5, pixels * 9, anchor='nw', image=self.obstacle2_object)
        # Obstacle 56
        self.obstacle56 = self.canvas_widget.create_image(pixels * 6, pixels * 9, anchor='nw', image=self.obstacle2_object)
        # obstacle 57
        self.obstacle57 = self.canvas_widget.create_image(pixels * 2, pixels * 2, anchor='nw', image=self.obstacle11_object)
        # obstacle 58
        self.obstacle58 = self.canvas_widget.create_image(0, pixels * 7, anchor='nw', image=self.obstacle11_object)
        # obstacle 59
        self.obstacle59 = self.canvas_widget.create_image(pixels * 1, pixels * 11, anchor='nw', image=self.obstacle11_object)
        # obstacle 60
        self.obstacle60 = self.canvas_widget.create_image(pixels * 8, pixels * 12, anchor='nw', image=self.obstacle12_object)
        # obstacle 61
        self.obstacle61 = self.canvas_widget.create_image(pixels * 13, pixels * 2, anchor='nw', image=self.obstacle12_object)
        # obstacle 62
        self.obstacle62 = self.canvas_widget.create_image(pixels * 5, pixels * 10, anchor='nw', image=self.obstacle13_object)
        # obstacle 63
        self.obstacle63 = self.canvas_widget.create_image(pixels * 10, 0, anchor='nw', image=self.obstacle13_object)


        # Goal Point
        img_halte = Image.open("images/halte.png")
        self.halte_object = ImageTk.PhotoImage(img_halte)
        self.flag = self.canvas_widget.create_image(pixels * 8, pixels * 11, anchor='nw', image=self.halte_object)

        # Uploading the image of bus
        img_bus = Image.open("images/bus.png")
        self.bus = ImageTk.PhotoImage(img_bus)

        # Creating an agent with photo of bus
        self.agent = self.canvas_widget.create_image(0, 0, anchor='nw', image=self.bus)
        ...
        if next_state == self.canvas_widget.coords(self.flag):
            ...
            if len(self.d) > self.longest:
                self.longest = len(self.d)
                            ...
                            self.canvas_widget.coords(self.obstacle53),
                            self.canvas_widget.coords(self.obstacle54),
                            self.canvas_widget.coords(self.obstacle55),
                            self.canvas_widget.coords(self.obstacle56),
                            self.canvas_widget.coords(self.obstacle57),
                            self.canvas_widget.coords(self.obstacle58),
                            self.canvas_widget.coords(self.obstacle59),
                            self.canvas_widget.coords(self.obstacle60),
                            self.canvas_widget.coords(self.obstacle61),
                            self.canvas_widget.coords(self.obstacle62),
                            self.canvas_widget.coords(self.obstacle63)]:
            reward = -1
            done = True
            next_state = 'obstacle'
